...
// pins for RGB LED
#define PIN_RED	 PB1   // OCR1B == red    == PB4 == pin3
// pins for USB
#define PIN_USBP PB4   // pin7 == USB D+ must be PB2 / INT0
#define PIN_USBM PB3   // pin2 == USB D-

#define setRedOut(x) ( OCR1B = (x) )
/* #define setGrnOut(x) ( OCR0A = 255 - (x) ) */
/* #define setBluOut(x) ( OCR0B = 255 - (x) ) */
extern void setPix(unsigned char r, unsigned char g, unsigned char b);
#define setRGBOut(r,g,b) { setPix(r,g,b); /* setGrnOut(g); setBluOut(b); */ }
...
static const uint8_t downrep[8]  = {REPID_KEYBOARD,8,0,0x2B,0,0,0,0}; // cmd+tab
...
int ensureUp() {
  if (usbInterruptIsReady()) {
    usbSetInterrupt(emptyrep, REPORT_COUNT); // send
    return 1;
  }
  return 0;
}
